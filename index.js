...
function getArtistByIndex(artist, index) {
  for (let i = 0; i < artist.length; i++) {
    if (i === index) {
      return `the artist at index ${[i]} is ${artist[i].name}`;
    }
  }
}
...
function get20s(artist) {
  const art20 = [];
  for (let i = 0; i < artist.length; i++) {
    let text = artist[i].years;
    const years = text.split("-");
    if (years[0] >= 1900 && years[1] <= 2000) art20.push(artist[i].name);
  }
  return art20;
}
...
function addArtist(artists) {
  artists.push("33", "shane", "web dev", "american", "lorem blah blah");
  return artists;
}
...
function lotsOfArt(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    if (array[i].paintings > 100) names.push(array[i].name);
  }
  return names;
}
...
function artistInfo(artists, name) {
  ...
  for (let i = 0; i < artists.length; i++) {
    if (artists[i].name === name) bio = artists[i].bio;
  }
  return [bio];
}
...
function artistByCountry(artists, nationality) {
  const list = [];
  for (let i = 0; i < artists.length; i++) {
    if (artists[i].nationality === nationality) list.push(artists[i].name);
  }
  return list;
}
